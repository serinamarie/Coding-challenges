...
A large amount of money was stolen today from the main city bank, and as the chief of police it's your duty 
to find the robber.
...
the robber left their signature near the crime scene: "B. Gre?n". "B" definitely stands for the first letter 
of robber's name, and "Gre?n" is their surname. The 4th letter of the surname is smudged by ketchup and is 
unreadable.
...
BEGIN
    ...
    FROM Suspect
    WHERE height <= 170
    OR LOWER(SUBSTRING(name, 1, 1)) != 'b'
    OR LOWER(SUBSTRING(surname, 1, 3)) != 'gre'
    OR LOWER(SUBSTRING(surname, 5, 1)) != 'n';
END